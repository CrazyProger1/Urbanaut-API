...
class Command(BaseCommand):
    help = "Generates an OTP authentication QR code for a specified user"

    def add_arguments(self, parser):
        parser.add_argument(
            "user",
            type=str,
            help="ID of the user to generate OTP QR code for",
        )
    ...
    def handle(self, *args, **options):
        ...
        url = create(
            key=settings.OTP_SECRET_KEY + str(user_id),
            name=str(user_id),
